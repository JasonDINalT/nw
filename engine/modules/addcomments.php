...
if( $config['yandex_spam_check'] AND $config['yandex_api_key'] ) {

	if( check_yandex_spam( array('ip' => $_IP, 'name' => $name, 'email' => $mail, 'key' => $config['yandex_api_key'], 'body-html' => $comments ) ) ) {
		$stop[] = $lang['news_err_46'];
		$CN_HALT = TRUE;		
	}
}
